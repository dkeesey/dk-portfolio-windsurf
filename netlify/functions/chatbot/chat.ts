...
export const handler: Handler = async (event: HandlerEvent) => {
    ...
    try {
        ...
        const { message, conversationId, recruiterId, authToken, guestMode = false } = body;
        ...
        const isGuestMode = guestMode || recruiterId.startsWith('guest-');
        
        // If in guest mode, we can skip database operations
        if (isGuestMode) {
            ...
            try {
                ...
                // Return the response without storing in database
                return {
                    statusCode: 200,
                    headers,
                    body: JSON.stringify({
                ...
                console.error('Error in guest mode chat completion:', error);
                return {
                    statusCode: 500,
                    headers,
                    body: JSON.stringify({
        ...
        // For non-guest mode, proceed with database operations
        if (!supabase) {
            console.warn('Supabase client not initialized. This is expected during build.');
            return {
                statusCode: 200,
                headers,
                body: JSON.stringify({
